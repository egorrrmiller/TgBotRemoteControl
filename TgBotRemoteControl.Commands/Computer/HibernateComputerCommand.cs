...
public class HibernateComputerCommand : ICommand
{
    public Task<bool> CanExecute(Update update)
    {
        if (update.CallbackQuery != null)
        {
            return Task.FromResult(update.CallbackQuery.Data.Contains(ControlEnum.Hibernate.ToString()));
        }
    ...
    public async Task Execute(Update update, ITelegramBotClient client)
    {
        ...
        for (int i = 3; i > 0; i--)
        {
            ...
            await client.EditMessageTextAsync(update?.CallbackQuery?.Message?.Chat.Id,
                ...
                $"Компьютер отправится в гибернацию через {i}...",
